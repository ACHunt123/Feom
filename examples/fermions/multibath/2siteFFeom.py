...
psi_0 = generate_state(2,[0,1])
rhos_0 = np.outer(psi_0.conj(),psi_0)
...
rho=sim.rho
rho11=np.array([np.trace(c_dags[0] @ c_s[0]@rho[it,:,:]) for it in range(len(sim.t_arr))])
trace=np.array([np.trace(rho[it,:,:]) for it in range(len(sim.t_arr))])

# get the qutip results
try:
    qutipdata=np.loadtxt('rho11qutip.dat')
    t_arr_qutip=qutipdata[:,0]
    rho11_qutip=qutipdata[:,1]
    trace_qutip=qutipdata[:,2]
except:
    print( ' no data from qutip - run it first silly!')

# plot
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
axes[0].plot(sim.t_arr, rho11, 'o', color='crimson', label='Fermionic HEOM', markersize=4)
...
try:
    ...
    print('align the t axes for error comparsion please')
    print(f'len t_arr = {len(sim.t_arr)}')
    print(f'len t_arr_qutip = {len(t_arr_qutip)}')
    print(f'len t_arr_qutip = {len(rho11_qutip)}')
plt.tight_layout()
plt.show()