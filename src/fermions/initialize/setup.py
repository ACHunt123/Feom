...
class Setup:
    ...
    def _validate_system(self, pot):
        ...
        """
        for k,V_mat in enumerate(pot.V_ks_plus):
        ...
        try: # Type safety
            pot.V_ks_plus = [np.array(V_k_plus, dtype=complex) for V_k_plus in pot.V_ks_plus]
            # calculate the V_ks lowering operators if not specified
            pot.V_ks_mnus = [np.array(V_k_plus.T.conj(), dtype=complex) for V_k_plus in pot.V_ks_plus]
    ...
    def _setup_bath(self, bath_dict):
        ...
        bath_obj.K = len(bath_obj.C_ks_plus)*2
    ...
    def _validate_bath(self, bath):
        ...
        """
        # Array lengths
        if len(bath.C_ks_plus) != len(bath.gam_ks_plus)!=len(bath.C_ks_mnus) != len(bath.gam_ks_mnus):
            raise ValueError(f"Bath Dimension Mismatch check the dims of gam_ks and C_ks for +-")
        # Type safety
        try:
            bath.C_ks_plus = np.array(bath.C_ks_plus, dtype=complex)
            bath.C_ks_mnus = np.array(bath.C_ks_mnus, dtype=complex)
            bath.gam_ks_plus = np.array(bath.gam_ks_plus, dtype=complex)
            bath.gam_ks_mnus = np.array(bath.gam_ks_mnus, dtype=complex)
        except Exception as e:
            raise TypeError(f"Could not convert bath arrays to complex numbers: {e}")
        # Zero-Length Check (Edge case)
        if len(bath.C_ks_plus) == 0:
